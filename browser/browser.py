...
#         """
#         text = ''
#         tags = ['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'a', 'ul', 'ol']
#         soup = BeautifulSoup(resp.content, 'html.parser')
#
#         for tag in soup.find_all(tags):
#             if tag.name == 'a':
#                 text += Fore.BLUE + tag.get_text('\n',True)
#             else:
#                 text += Fore.BLACK + tag.get_text('\n', True)
#         return text
#
...
#         # browser loop
#         while True:
#             user_input = input('Type domain or "exit"-> exit: ')
#
#             if user_input == 'exit':
#                 deinit()
#                 break
...
#
import requests
resp = requests.get('https://docs.python.org')
from bs4 import BeautifulSoup
from colorama import Fore
    ...
    for tag in soup.find_all(tags):
        for string in tag.stripped_strings:
            if string not in text:
                    # text.append(string)
                if tag.name == 'a':
                    if Fore.BLUE + string not in text:
                        text.append(Fore.BLUE + string)
                else:
                    if Fore.BLACK + string not in text:
                        text.append(Fore.BLACK + string)


    return '\n'.join(text)

print(parse_html(resp))
